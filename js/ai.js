...
// Bug utente (settembre 2026): "la parte AI di narrazione non parte mai". Causa reale, confermata
// sui Runtime Logs di Vercel (get_runtime_errors su questo progetto): 15 occorrenze negli ultimi 7
// giorni di "Vercel Runtime Timeout Error: Task timed out after 300 seconds" proprio su /api/ai --
// il fetch verso Gemini qui sotto non aveva NESSUN timeout, quindi se Gemini non rispondeva (rete,
// throttling, quota) la funzione restava appesa fino al limite duro della piattaforma (300s). Nel
// frattempo lato client (index.html, narrateAttack/narrateMovement) la UI aspettava solo 6 secondi
// prima di rinunciare e mostrare la frase base -- quindi anche nei casi in cui Gemini rispondeva
// correttamente ma non entro 6s, la frase generata veniva comunque scartata. Risultato: la
// narrazione AI non si vedeva mai, né negli errori veri né nelle risposte solo "lente".
// Fix qui: un AbortController con timeout esplicito (25s, ben sotto il limite Vercel Hobby di 300s
// e i 60s del piano Pro) così una chiamata bloccata fallisce in modo pulito con un errore leggibile
// invece di restare appesa fino al kill duro della piattaforma. Il fix lato client (aumentare la
// finestra di attesa della UI oltre i 6s) è in index.html, non qui.
...
async function fetchWithTimeout(url, options, timeoutMs) {
  ...
  try {
    ...
    if (e && e.name === 'AbortError') {
      throw new Error(`Timeout: Gemini non ha risposto entro ${Math.round(timeoutMs / 1000)}s`);
    }
    throw e;
  } finally {
    clearTimeout(timer);
...
async function handleCorrect(req, res) {
  ...
  const r = await fetchWithTimeout(geminiUrl(), {
    method: 'POST',
