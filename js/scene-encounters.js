...
//
// window.__dvosRefreshDexPanel(canEdit): il pannello "Digidex" (dex-live) resta nell'IIFE di
// index.html (fa parte del futuro "Pannello Dex admin", non ancora estratto) ma va aggiornato da
// qui in due punti (aggiunta rapida al Dex da un Incontro, apertura del dettaglio Incontro) — sia
// da funzioni con un vero call-site esterno sia dal listener di click globale qui sotto, che si
// auto-installa al caricamento dello script (prima ancora che l'IIFE di index.html giri). Per
// questo non è un parametro esplicito ma un hook globale, sullo stesso modello già usato in
// index.html per window.__dvosNotify/window.__dvosSession.
...
// ---------- helper generici (spostati da index.html) ----------
    ...
    live.querySelectorAll('[data-quickdex]').forEach(btn=>{
      btn.onclick = async ()=>{
        ...
        if(d && d.entry){
          ...
          live.innerHTML = encountersEditableHTML(cachedScene.encounters);
          bindEncountersInnerActions(code, username, onChanged);
          if(typeof window.__dvosRefreshDexPanel==='function') window.__dvosRefreshDexPanel(true);
        }
      };
    });
  }
  ...
  async function openEncounterDetail(name, code, username){
    ...
    try{
      ...
      if(typeof window.__dvosRefreshDexPanel==='function') window.__dvosRefreshDexPanel(session && session.role==='master');
