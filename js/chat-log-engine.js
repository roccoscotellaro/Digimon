...
async function patchMember(code, username, digimonPatch, tamerPatch){
      ...
      if(fulfillTormentBtn && me){
        ...
        if(tor && !tor.usedThisRest){
          ...
          if(result.outcome==='crit-success'){
            ...
            tor.usedThisRest = true;
            // Stessa identica scelta di js/tamer-card.js (data-torment-check) -- vedi il commento
            // lì per il dettaglio della regola (2.05b) e del blocco reale in trySpendAction
            // (index.html, participant.tamerActionsLocked).
  ...
  function formatLogText(raw){
    ...
    // Richiesta utente ("come per lo scherzo, aggiungerei una nota e un avviso del Master"): due
    // marcatori nuovi, STESSO meccanismo dello scherzo qui sopra (estratti dal flusso, sostituiti
    // con un <details> chiuso di default — un clic per aprirlo — niente testo grezzo in mezzo alla
    // narrazione) ma due funzioni distinte, come chiarito da Rocco:
    //   "(nota: testo)"   -> nota generica: chiunque (giocatore o Master) può scriverla per un
    //                        piccolo appunto/OOC a margine del messaggio. Chip neutro (ciano,
    //                        come i mention-chip) per non confondersi né con lo scherzo (ambra)
    //                        né con l'avviso del Master (rosso, sotto).
    //   "(avviso: testo)" -> avviso del Master: stesso "chiuso di default, un clic per aprire"
    //                        (Rocco ha confermato di volere la stessa UX dello scherzo, non
    //                        sempre visibile), ma il chip CHIUSO usa il colore --danger apposta
    //                        per risaltare anche prima di essere aperto — un avviso importante
    //                        (una regola, un evento) non deve perdersi nella massa dei messaggi
    //                        come farebbe un chip neutro.
    // Nessun controllo di ruolo qui (chi può scrivere cosa): come lo scherzo, sono pure convenzioni
    // testuali riconosciute da chiunque le scriva — coerente con l'assenza di permessi reali sul
    // contenuto dei messaggi nel resto del sito (vedi log.js). Vanno PRIMA dell'evidenziazione
    // delle virgolette, stesso motivo dello scherzo sopra.
    html = html.replace(/\(\s*nota\s*[:,]?\s*([^<)]+)\)/gi, (m, note)=>{
      const noteText = note.trim();
      if(!noteText) return m;
      return `<details class="log-note"><summary>📝 nota</summary><span class="log-note-text">${noteText}</span></details>`;
    });
    html = html.replace(/\(\s*avviso\s*[:,]?\s*([^<)]+)\)/gi, (m, warn)=>{
      const warnText = warn.trim();
      if(!warnText) return m;
      return `<details class="log-warning"><summary>📌 avviso del Master</summary><span class="log-warning-text">${warnText}</span></details>`;
    });
