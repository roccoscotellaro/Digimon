  ...
  function renderDigimonCard(me, containerId, onChanged, renderTamerCardFn){
    ...
    if(!editingDigimon){
      cardEl.innerHTML = `
        ...
        <div class="row">
          <select id="evo-target-${containerId}" style="flex:2;">${STAGES.map(s=>{
            const built = (d.stageStats && d.stageStats[s]) || s===d.stage;
            return `<option value="${s}" ${s===d.stage?'selected':''}>${s}${built?'':' (da costruire)'}</option>`;
          }).join('')}</select>
      ...
      if(sigBtn){
        sigBtn.onclick = async ()=>{
          ...
          await saveMember(session.code, me);
          await pushLog(session.code, { who: me.digimon.name||displayName(me), role:'roll', text: `⚡ ${me.digimon.name||'Il Digimon'} usa la Signature Move! Bonus da Battery: +${spent} Accuracy/Damage (o +${spent} da dividere tra Accuracy e Potenza Effetto se [SUPPORT]) su questo attacco.` });
          renderDigimonCard(me, containerId, onChanged, renderTamerCardFn);
          if(onChanged) onChanged();
        };
      }
      ...
      if(evoBtn){
        evoBtn.onclick = async ()=>{
          ...
          await saveMember(session.code, me);
          await pushLog(session.code, { who: displayName(me), role:'player', text: `✨ ${me.digimon.name||'Il Digimon'} è avvolto da un bagliore di dati... e digivolve in ${target}!${cost>0?` (spesi ${cost} Evolution Points)`:''} — Stat aggiornate automaticamente. Ricorda di segnare 1 Azione spesa.` });
      ...
      if(forceBtn){
        forceBtn.onclick = async ()=>{
          ...
          if(verdict.cls==='crit-success'){
            ...
            me.digimon.currentWounds = me.digimon.maxWounds;
            outcomeText = `✨ Successo Critico! ${me.digimon.name||'Il Digimon'} è scosso da un'ondata di energia incontrollabile e digivolve, tornando subito al Default Stage (${me.digimon.defaultStage}) — Stat aggiornate.`;
          } else if(verdict.cls==='success'){
            applyStageChange(me.digimon, me.digimon.stage, target);
            me.digimon.currentWounds = me.digimon.maxWounds;
            me.digimon.forcedEvoRoundsLeft = 3;
            outcomeText = `✨ Successo! ${me.digimon.name||'Il Digimon'} è avvolto da un bagliore instabile e digivolve in ${target} per 3 round (Stat aggiornate), poi tornerà sotto il Default Stage.`;
          } else {
            outcomeText = `${verdict.label}. Nessuna evoluzione forzata — possibili conseguenze pericolose a discrezione del Master.`;
          }
          await saveMember(session.code, me);
          await pushLog(session.code, { who: displayName(me), role:'roll', text: `⚡ Forza Evoluzione (TN ${tn}): 3d6[${dice.join(',')}] + Willpower ${willVal} = ${total} → ${outcomeText}`, meta:{ dice } });
      ...
      if(toggleEvolveBtn){
        toggleEvolveBtn.onclick = async ()=>{
          ...
          await saveMember(session.code, me);
          await pushLog(session.code, { who:'Master', role:'gm', text: me.digimon.evolveUnlocked ? `${me.digimon.name||'Il Digimon'} di ${displayName(me)} sente una strana energia provenire dall'interno...` : `${me.digimon.name||'Il Digimon'} di ${displayName(me)} non sente più quella strana energia (Evoluzione ribloccata dal Master).` });
          renderDigimonCard(me, containerId, onChanged, renderTamerCardFn);
          if(onChanged) onChanged();
        };
      }
      ...
      if(blastBtn){
        blastBtn.onclick = async ()=>{
          ...
          await saveMember(session.code, me);
          await pushLog(session.code, { who: displayName(me), role:'roll', text: `💥 Blast Evolution a ${target} — Willpower Check (TN ${tn}): 3d6[${dice.join(',')}] + Willpower ${willVal} = ${total} → ${outcomeText} (Usi rimasti: ${me.digimon.blastEvolutionUses})`, meta:{ dice } });
          statusEl.style.color='var(--text-mute)'; statusEl.textContent = outcomeText;
          renderDigimonCard(me, containerId, onChanged, renderTamerCardFn);
          if(renderTamerCardFn) renderTamerCardFn(me, siblingCardId(containerId,'tamer'));
          if(onChanged) onChanged();
      ...
      if(slideBtn){
        slideBtn.onclick = async ()=>{
          ...
          await saveMember(session.code, me);
          await pushLog(session.code, { who: displayName(me), role:'player', text: `🔀 ${me.digimon.name} usa Slide Evolution (stesso Stage, 1 Evolution Point speso)${woundDiff!==0?`, Ferite Massime ${woundDiff>0?'+':''}${woundDiff}`:''}.` });
      ...
      if(darkEndBtn){
        darkEndBtn.onclick = async ()=>{
          ...
          await saveMember(session.code, me);
          await pushLog(session.code, { who:'Sistema', role:'gm', text: `${me.digimon.name} rientra dalla Dark Evolution, tornando a ${me.digimon.defaultStage}.` });
      ...
      document.querySelectorAll(`[data-armor-use]`).forEach(btn=>{
        btn.onclick = async ()=>{
          ...
          await saveMember(session.code, me);
          await pushLog(session.code, { who: displayName(me), role:'player', text: `${me.digimon.name||'Il Digimon'} usa il Digimental "${af.digimentalName}" per Armor Evolvere in ${af.name} (${af.stage}, 0 Evolution Points).` });
          renderDigimonCard(me, containerId, onChanged, renderTamerCardFn);
          if(onChanged) onChanged();
        };
      });
      document.querySelectorAll(`[data-armor-revert]`).forEach(btn=>{
        btn.onclick = async ()=>{
          ...
          await saveMember(session.code, me);
          await pushLog(session.code, { who: displayName(me), role:'player', text: `${me.digimon.name||'Il Digimon'} torna al Default Stage (${me.digimon.defaultStage}), lasciando la forma ${af.name}.` });
          renderDigimonCard(me, containerId, onChanged, renderTamerCardFn);
          if(onChanged) onChanged();
        };
      });
      cardEl.querySelectorAll('[data-pool]').forEach(btn=>{
        btn.onclick = async ()=>{
          ...
          await pushLog(session.code, { who: (d.name||displayName(me)), role:'roll', text: logText, meta: { dice, successes } });
          if(onChanged) onChanged();
        };
      });
    } else {
